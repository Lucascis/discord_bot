...
| --- | --- | --- | --- | --- |
| **Hydra** | [hydra.bot](https://hydra.bot/) | Slash-command driven playback, per-guild DJ roles, web dashboard, powered by a proprietary audio cluster. | Tier de pago desbloquea modo 24/7, volumen global, filtros de audio. | Runs a multi-region Lavalink cluster; REST dashboard exposes queue management. |
...
| **Chip Bot** | [chipbot.gg](https://chipbot.gg/) | Multi-source playback with waveform filters, persistent autoplay queues, uptime guarantees published on a public status page. | Tier de pago desbloquea modo lossless, nodos prioritarios, bundles. | Provides customer-facing SLA + transparent incident postmortems; emphasises SOC2-compliant payment flows. |
...
3. **Transparent monetization messaging matters.** Hydra and Green-Bot clearly describe which features move behind paid tiers. Our README now explicitly states that only a stub payment provider is wired until Stripe/MercadoPago are provisioned, avoiding mismatched expectations.
...
1. **Operational transparency.** Chip Bot’s public status page and incident write-ups underline how critical clear communications are for customers. Our monitoring stack (Prometheus + OTEL) should feed a lightweight `/status` surface and the deployment guide now emphasises alert routing so that outages are observable.
2. **Upgrade UX.** Competitors push upgrade CTAs directly in the dashboard. Because tiers now load strictly from the database, we can model campaign experiments (intro pricing, seasonal limits) without redeploying code. Adding a “overrides” admin panel is the next logical UX task.
...
4. **Playback fidelity.** Hydra/Chip advertise multi-region, low-latency nodes. Our gateway now fails fast when plans aren’t hydrated; the follow-up is to extend the Lavalink provisioning scripts so regional clusters can be toggled per tier directly from the DB.

Tracking these deltas keeps the bot focused on user-facing wins (dashboards, stability, operational clarity) rather than feature sprawl.

## Web & Panel Experience Strategy

### Visual/Landing Inspiration
- **Hydra** apuesta por un landing hero oscuro con ilustraciones sintéticas, destaca botones primarios brillantes y cards con “Unlock” muy visibles. La navegación es mínima (Home / Commands / Status) y cada sección muestra capturas del panel.
...
**Dirección para nuestro sitio/panel:** usar un tema oscuro neon (morado/azul) que evoque música en vivo, hero con visualización de onda y CTA dual (“Probar gratis” / “Panel avanzado”). Incluir badges de desempeño (“99.99% uptime”, “Lavalink regional”) y screenshots reales del dashboard.

### Funcionalidades del Panel Administrativo
| Área | Debe Tener | Extras Diferenciadores |
|------|------------|------------------------|
| **Control de niveles** | Listado de `subscription_plans`/`subscription_prices`, filtros por proveedor, edición inline y recarga (`/api/v1/plans`, `/runtime`, `/reload`). | **Experimentos**: banderas para campañas (ej. “Black Friday price id”), vista previa de landing (cómo se ve cada nivel público). |
| **Gestión de guilds** | Estado de suscripción por servidor, límites consumidos (tracks, queue), logs de facturación. | **Playbooks automáticos**: sugerencias basadas en uso (p.ej. “este guild llega al límite de filtros, upsell a nivel avanzado”). |
| **Soporte/Reembolsos** | Acción rápida para pausar/cancelar suscripción, ver historial de pagos. | **Botón “Compensar tiempo”** que ajusta metadatos operativos con presets (1 semana, 1 mes). |
...
1. **Hero** – headline claro (“Música para Discord sin fricción”), CTA dual, badge de uptime y slider de logos.
2. **Demostración interactiva** – mock del panel mostrando queue analytics, toggles de features.
3. **Niveles** – cards animadas conectadas a la DB (usar `/api/v1/plans/runtime`). Botón “Ver detalles” abre modal con limits real-time.
...
6. **FAQ + CTA final** – preguntas sobre seeds de niveles, integración Stripe/MercadoPago, flows de guilds grandes.

### Diferenciadores UX
- **Control centralizado 100% DB**: ningún fallback en código; el panel puede programar promociones temporales sin redeploy. Mostrarlo como “motor de configuración”.
...
4. Añadir módulo de experimentos (feature flags) ligado a metadata de niveles.
...
1. **Landing pública**
   ...
   - Tabla de niveles conectada a `/api/v1/plans/runtime`.
   - Comparativa vs competencia y timeline de status/SLAs.
2. **Portal de usuarios**
...
3. **Panel interno (staff)**
  - Motor de configuración (CRUD + experiments) → `/api/v1/plans`, `/reload`.
  - Guild Monitor: lista de guilds con KPI, eventos de facturación.
   - Health & incident board (enlaces a Prometheus/Sentry y timeline).

### Componentes Clave
| Componente | Datos | Acciones |
|------------|-------|----------|
| `TierCard` | `GET /plans/runtime` | CTA upgrade, detalla limits/features. |
| `TierEditorModal` | `GET/POST /plans` | Edita nombres, features, IDs de precios; guarda cambios y dispara `/reload`. |
...
| `ExperimentToggle` | Metadatos en `subscription_plans.features.experiments` (tabla DB) | Activa campañas de precio/beneficios. |
...
- Auditoría: guardar cambios de niveles/guild settings en tabla `admin_audit_logs` (timestamp, actor, diff).

### Roadmap de Desarrollo
| Fase | Duración estimada | Entregables |
|------|-------------------|-------------|
| **Fase 0 – Setup** | 1 semana | Repo `apps/panel`, Next.js + Auth boilerplate, CI (lint/test). |
| **Fase 1 – Panel público** | 2 semanas | Landing con hero, comparativa, tabla de niveles viva, enlaces a status. |
| **Fase 2 – Portal usuarios** | 3 semanas | Dashboard de guild, Studio Mode v1, integración OAuth y Stripe portal. |
| **Fase 3 – Panel interno** | 3 semanas | CRUD de niveles, experiments, monitoreo health & incident timeline. |
...
### Estado actual del motor de configuración (Fase 2)
- `/admin/plans` ya expone una capa inspirada en los paneles de Hydra/Green-Bot: lista la metadata viva desde PostgreSQL y ofrece edición inline de nombre, descripción, flags de experimentos y precios activos.
