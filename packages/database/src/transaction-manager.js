...
export class TransactionManager {
    ...
    async withTransaction(operations, options = {}) {
        const { maxWait = 5000, timeout = 10000, isolationLevel = 'ReadCommitted', retryAttempts = 3, retryDelay = 1000 } = options;
        ...
        for (let attempt = 0; attempt <= retryAttempts; attempt++) {
            ...
            try {
                ...
                if (!isRetryable || attempt === retryAttempts) {
                    ...
                    metrics.duration = metrics.endTime - metrics.startTime;
                    this.activeTransactions.delete(transactionId);
                    throw new TransactionError(`Transaction failed after ${attempt + 1} attempts: ${lastError.message}`, this.getErrorCode(lastError), isRetryable && attempt < retryAttempts);
                }
            }
        }
     ...
     */
    async withParallelTransactions(transactions, options = {}) {
        const transactionPromises = Object.entries(transactions).map(async ([key, operation]) => {
            try {
                const result = await this.withTransaction(operation, options);
                return { key, result, error: null };
            }
            catch (error) {
                return { key, result: null, error: error };
            }
        });
        ...
        if (failures.length > 0) {
            ...
            throw new TransactionError(`${failures.length}/${results.length} parallel transactions failed`, 'PARALLEL_TRANSACTION_FAILURE');
        }
        const finalResult = {};
    ...
    async withSaga(steps, options = {}) {
        ...
        try {
            ...
            for (let i = completedSteps.length - 1; i >= 0; i--) {
                ...
                try {
                    await this.withTransaction((tx) => step.compensate(tx, result), { ...options, retryAttempts: 1 });
                    getLogger().info({
                        stepName: step.name,
    ...
    createOperationTracker(metrics) {
        return (operation) => {
...
export function getTransactionManager(prisma) {
...
export const TransactionPatterns = {
     ...
     */
    async atomicQueueUpdate(tx, guildId, operations) {
     ...
     */
    async atomicAuditLog(tx, guildId, userId, action) {
     ...
     */
    async atomicFeatureFlagUpdate(tx, guildId, flagName, enabled, userId) {
        // Update feature flag
