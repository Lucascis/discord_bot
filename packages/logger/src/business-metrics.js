...
export class BusinessMetricsCollector {
    ...
    trackSongSkip(guildId, track, playedDuration, reason, userId) {
    ...
    // Queue tracking
    trackQueueOperation(guildId, operation, queueLength, userId) {
    ...
    // Autoplay tracking
    trackAutoplayTrigger(guildId, triggerType, userId) {
        this.metrics.autoplayTriggers.labels(guildId, triggerType).inc();
        if (userId) {
            this.trackUserActivity(userId, guildId);
        }
    }
    trackAutoplayRecommendation(guildId, recommendationType, success, trackTitle, userId) {
        if (success) {
