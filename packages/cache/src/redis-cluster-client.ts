...
export interface RedisClusterConfig {
  ...
  // Redis options (applied to all nodes)
...
export class RedisClusterClient {
  ...
  private readonly config: Required<RedisClusterConfig>;
  ...
  private setupEventHandlers(): void {
    ...
    this.cluster.on('+node', (node: Redis) => {
      logger.info({
  ...
  private async updateClusterInfo(): Promise<void> {
    try {
    ...
    if (this.circuitOpen) {
      ...
      throw new Error(
        `Circuit breaker is open. Retry after ${
          Math.ceil((this.config.circuitBreakerTimeout - elapsed) / 1000)
        }s`
      );
    }
  }
  ...
  async executeCommand<T>(
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        const result = await this.cluster[command](...args) as T;
  ...
  async getStats(): Promise<ClusterStats> {
    const nodes = this.cluster.nodes('all');
    const nodeStats: ClusterNodeStats[] = [];
    ...
    for (const node of nodes) {
      try {
        ...
        if (role === 'master') masterNodes++;
        if (role === 'slave') slaveNodes++;

        const status = node.status === 'ready' ? 'connected' :
                      node.status === 'connecting' ? 'connecting' :
                      'disconnected';

        if (status === 'connected') connectedNodes++;
 ...
 */
export function parseClusterNodes(nodesString: string): ClusterNode[] {
