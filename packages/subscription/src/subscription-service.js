...
export class SubscriptionService {
     ...
     * Update subscription
     */
    async updateSubscription(guildId, params) {
     ...
     * Cancel subscription
     */
    async cancelSubscription(guildId, reason, immediately = false) {
        const subscription = await this.prisma.subscription.findUnique({
            where: { guildId },
        });
        if (!subscription) {
     ...
     */
    async checkFeatureAccess(guildId, featureKey) {
     ...
     * Check usage limit
     */
    async checkUsageLimit(guildId, limitType) {
        const subscription = await this.getSubscription(guildId);
     ...
     */
    async incrementUsage(guildId, limitType, amount = 1) {
        const subscriptionRecord = await this.prisma.subscription.findUnique({ where: { guildId } });
        if (!subscriptionRecord)
            return;
     ...
     */
    async updateUsageTracking(guildId, updates) {
        const subscriptionRecord = await this.prisma.subscription.findUnique({
            where: { guildId },
            include: { usageTracking: true },
        });
        if (!subscriptionRecord || !subscriptionRecord.usageTracking)
            return;
     ...
     */
    async initializeUsageLimits(subscriptionId, tier) {
        const limitKeys = [
     ...
     */
    async updateUsageLimits(subscriptionId, newTier) {
     ...
     * Initialize usage tracking
     */
    async initializeUsageTracking(subscriptionId, periodStart, periodEnd) {
     ...
     * Log subscription event
     */
    async logSubscriptionEvent(guildId, eventType, tier, previousTier) {
